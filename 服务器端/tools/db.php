...
class db
{
    ...
    {
        $this->pdo = new \PDO("mysql:host=" . DNC_DB_HOST . ";dbname=" . DNC_DB_NAME, DNC_DB_USER, DNC_DB_PWD);
        $this->pdo->query("set names " . DNC_DB_CHARSET);
    }

    /**
     * 克隆，强制复制一份this->object， 否则仍然指向同一个对象
     */
    ...
    {
        $tableName = DNC_DB_PRE . $tableName;
    ...
    public function add($data = null)
    {
        ...
        if (!is_array($data)) {
            $this->showError('插入数据错误', '插入数据应为一个一维数组');
        }
    ...
    {
        if (empty($this->where)) {
            $this->showError('危险操作 (无条件删除).', '请使用模型对象的where()函数设置删除条件');
        }
        $where = $this->getWhere();
    ...
    public function update($data = null)
    {
        ...
        if (empty($data) || !is_array($data)) {
            $this->showError('更新数据错误.', 'update($data) 函数的参数应该为一个一维数组');
        }
        if (empty($this->where)) {
            $this->showError('危险操作 (无条件更新).', '请使用模型对象的 where() 方法设置更新条件');
        }
        $where = $this->getWhere();
