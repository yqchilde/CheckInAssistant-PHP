	...
	<view>
        ...
        <view class="top u-f-ac u-f-column">
            ...
            <view class="item u-f-ajc u-f-jsb">
                ...
                <text class="top-right">按{{signInWayTxt}}方式签到</text>
            </view>
        </view>
        
        
        <!-- 创建者打开且为签到码签到方式 -->
        <template v-if="openMode=='create' && signInWay == 'code'">
            <view class="signCode u-f-ac u-f-column">
                <view class="label">签到码</view>
                <view class="flex">
                    <view v-for="(item, index) in signCode" :key="index" class="grid flex align-center justify-center">{{item}}</view>
                </view>
                <view class="signCode-btn u-f-ajc" @tap="viewSign(signId)">
                    查
                </view>
            </view>
        </template>
        <!-- 创建者打开且为GPS签到方式 -->
        <template v-if="openMode=='create' && signInWay == 'gps'">
        ...
        <!-- 参与者打开且为签到码签到方式 -->
        <template v-else-if="openMode=='join' && signInWay == 'code'">
            ...
            <view v-if="signStatus == 0" class="u-f-ajc">
                <view class="btnStart u-f-ajc" @tap="toSignIn(signId)">
                    立即签到
                </view>
            </view>
            <view v-else-if="signStatus == 1 || signStatus == 3" class="u-f-ajc">
                <view class="btnEnd u-f-ajc u-f-column" @tap="viewSign(signId)">
                    已签到
        ...
        <!-- 参与者打开且为GPS签到方式 -->
        <template v-else-if="openMode=='join' && signInWay == 'gps'">
            <uni-notice-bar :scrollable="true" :single="true" :speed="50" :text=dizhi />
            <view class="signCode u-f-ac u-f-column" style="margin-top: 50rpx;">
	...
	export default {
		...
		data() {
			return {
                ...
                codeSign:'' ,//连续的签到码
                inputCode:'' ,//输入的数字
                signStatus:0 ,//签到状态（0表示待签到、1表示已签到、2表示签到结束、3表示时间结束且已签到、4表示不在范围内）
		...
		methods: {
			...
			getSignInfo(signId) {
                ...
                uni.request({
                    url: that.apiServer + 'sign&m=getSignInfo',
                    method: 'POST',
                    ...
                    success: res => {
                        ...
                        //拼接时间
                        let dates = timestampToTime.timestampToTime(res.data.beginTime, 2);
                        let times = timestampToTime.timestampToTime(res.data.beginTime, 3);
                        let signTime = timestampToTime.timestampToTime(res.data.signTime, 3);
                        this.times = dates + '' + times + '-' + signTime;
                        //倒计时时间
                        this.totalTime = parseInt(res.data.signTime - Math.floor(new Date / 1000));
                        this.checking();
                        uni.hideLoading();
                        //签到方式
                        switch (res.data.signInWay){
                            case 'code':
                                this.signInWay = 'code';
                                this.signInWayTxt = '签到码'
                                // 正则处理签到码
                                let code = res.data.signCode;
                                this.codeSign = code;
                                let regexp=/\d{1}/g, match;
                            ...
                            case 'gps':
                                this.signInWay = 'gps';
                                this.signInWayTxt = 'GPS'
                                this.latitude = res.data.latitude;
                                this.longitude = res.data.longitude;
                                //获取原来签到的目的地
                                var point = new plus.maps.Point(res.data.longitude, res.data.latitude);
            ...
            //点击签到
            toSignIn(signId) {
                var signInWay = this.signInWay;
                switch (signInWay){
                    case 'code':
                        if (this.inputCode === this.codeSign) {
                            this.signOrder(signId);
                        } else {
                            uni.showToast({title:'签到码错误',icon:'none'});
                            return;
                        }
                        break;
                    case 'gps':
            ...
            signOrder(signId) {
                let userId = uni.getStorageSync('USERID');
                uni.request({
                    url: that.apiServer + 'sign&m=toSignIn',
                    method: 'POST',
                    header: {'content-type': 'application/x-www-form-urlencoded'},
                    data: {signId,userId},
                    success: res => {
            ...
            viewSign(signId) {
                uni.navigateTo({
                    url:'../viewSign/viewSign?signId='+signId
                })
            },
            // 计算两个经纬度之间的距离
            computedDt(lon,lat) {
